...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.maxPool(x)
        #####################################
        x = self.activation2(x) ##  Splinformer  Prinformer
        #####################################
        #x = self.maxPool(y+x)  # Prinformer
        ## x = self.norm(x)
        x = x.transpose(1,2)
        return x
...
class Encoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.conv_layers is not None:
            ...
            # if len(self.attn_layers)==3: ### Informer  
            ...
            #     attns.append(attn)
            # if len(self.attn_layers)==3: ### Informer, Informer 
            #     x, attn = self.attn_layers[0](x, attn_mask=attn_mask)
            #     x = self.conv_layers[0](x)
            #     attns.append(attn)
            #     #x, attn = self.attn_layers[1](x, attn_mask=attn_mask) # Informer 
            #     x = self.conv_layers[1](x)
            #     attns.append(attn)
            # if len(self.attn_layers)==3: ### Informer, Informer 
            #     x, attn = self.attn_layers[0](x, attn_mask=attn_mask)
            ...
            #     attns.append(attn)
            #     x, attn = self.attn_layers[1](x, attn_mask=attn_mask)
            #     #x = self.conv_layers[1](x)  ## Informer 
            #     attns.append(attn)
            if len(self.attn_layers)==3: ### Informer, Informer  
                x, attn = self.attn_layers[0](x, attn_mask=attn_mask)
                x = self.conv_layers[0](x)
